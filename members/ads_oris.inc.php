...
echo "<h3  style=\"text-align: center;\">Vypis lidi, kteri jsou v members a jejich udaje z ORISu pro rok ".$ORIS_year."</h3>";
...
$data_tbl->set_header_col_with_help($col++,' Oris registracka',ALIGN_CENTER,"Nactena registracka z orisu");
$data_tbl->set_header_col_with_help($col++,' Oris jmeno',ALIGN_CENTER,"Nactene jmeno z orisu");
$data_tbl->set_header_col_with_help($col++,' Oris SI',ALIGN_CENTER,"Nactene SI z orisu");
...
while ($zaznam=mysqli_fetch_array($vysledek))
{
	...
	$row[] = '';
	$zbmreg = "ZBM".$reg;
	if (array_key_exists($zbmreg, $arr_oris["user"]))
	{
		$row[] = $arr_oris["user"][$zbmreg]->getReg();
		$row[] = $arr_oris["user"][$zbmreg]->getLastName()." ".$arr_oris["user"][$zbmreg]->getFirstName();
		$oris_si = $arr_oris["user"][$zbmreg]->getSI();
		if ($oris_si != $zaznam['si']) $oris_si = "<font style='color:red'>".$oris_si."</font>";
		$row[] = $oris_si;
		$arr_oris["members"][$zbmreg]=1;
	}
	else
	{
		$row[] = '';
...
echo "<h3  style=\"text-align: center;\">Vypis lidi, kteri jsou registrovani v ORISu a nejsou v members podle registracky</h3>";
