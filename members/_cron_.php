...
*/		
		...
		if ($zaznam_m['active_tf'])
		{
			$active = false;
			$lines = array();
//			echo("Aktivní - Blížící se konec termínu pøihlášek<br>\n");
...
//						echo('* Zavod - '.$races[$ii]['nazev'].' - vice se priblizil termin prihlasek<br>');
			...
			{
				$full_msg .= 'Blíží se jeden nebo více termínu pøihlášek: '.EMAIL_ENDL;
...
//						echo('* Zavod - '.$races[$ii]['nazev'].' - vcera byl termin prihlasek<br>');
		...
		if ($send_email)
		{
			...
			$full_msg .= 'Zmìnu a pøípadné zrušení zasílaných informací provedete pøes pøihláškový systém oddílu '.$g_shortcut.'.'.EMAIL_ENDL;
