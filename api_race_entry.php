...
if ($userSelected)
{
	...
	switch ($action) {
		case 'participate':
			...
			$query="UPDATE ".TBL_ZAVXUS." SET participated = not(participated) where id_zavod = $race_id and id_user = $id_user";
			@$result=$db_conn->query($query);
			$data = $db_conn->affected_rows;
			break;
		case 'addByFin':
			//id_race=X&id_user=X&action=addByFin
			$value = $_GET['value'];
			switch ($value) {
				case 'insert':
				...
				case 'delete':
					$query="DELETE FROM ".TBL_ZAVXUS." WHERE id_user = '$id_user' and id_zavod = '$race_id' and add_by_fin = 1;";
					@$result=$db_conn->query($query);
					$data = $db_conn->affected_rows;
					break;
					
				default:
					// console_log('addByFin without any action selected');
					$data = '';
					break;
			}
			break;
		...
		default:
			...
			$query="select * from ".TBL_ZAVXUS." z where id_zavod = $race_id and id_user = $id_user";
			@$result=$db_conn->query($query);
