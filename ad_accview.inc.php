...
@$list_v=MySQL_Query("SELECT id,login,podpis,policy_news,policy_regs,policy_mng,locked,last_visit FROM ".TBL_ACCOUNT." ORDER BY id");
...
{
	echo $data_tbl->get_new_row($list_z["id"], $list_z["login"], (($list_z["policy_news"]) ? "A" : "-"), (($list_z["policy_regs"]) ? "A" : "-"), (($list_z["policy_mng"]) ? (( $list_z["policy_mng"] == _MNG_BIG_INT_VALUE_) ? "A" : "m") : "-"), $list_z["podpis"], (($list_z["locked"]) ? "A" : "-"), Date2String($list_z["last_visit"]))."\n";
}
